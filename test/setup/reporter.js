...
export default class Reporter {
    ...
    constructor(runner, mocha) {
        ...
        if (/no_try_catch/.test(window.location.search)) {
          mocha.allowUncaught();
        }
    }
    ...
    setupDOM() {
        ...
        this.$stats.find('#no-try-catch')
          .attr('checked', /no_try_catch/.test(window.location.search))
    ...
    updateNoTryCatch() {
        if (this.$stats.find('#no-try-catch').is(':checked')) {
            if (!/[?&]no_try_catch/.test(window.location.search)) {
                window.location.search = '?no_try_catch';
            }
        } else {
            if (/[?&]no_try_catch/.test(window.location.search)) {
                window.location.search = '';
...
<ul id="mocha-stats">
    <li class="test-option" style="display: none;">
        <label>
